...
<template>
  <div>
   ...
   <table class="table">   
     ...
     <filtrosDivisoft v-show="activaFiltro" :header="locallastheader"  :isSidebarActive="activaFiltro" :filas="locallaststrFilas" :filtros="filtros"  :ObjectID=8105   @closeSidebar="activaFiltro = false" 
...
export default {
    ...
    beforeMount() {
        ...
        this.columnDefs = datos8105.columnDefs;
        var defaultdatos = datos8105.datosDefault;
    ...
    methods: {
        ...
        onDoubleClicked(event) {
            ...
            this.dinamicKey = [];
 this.dinamicKey.push({ nombre: "REFERENCIA", valor: event.node.data.REFERENCIA });
      ...
      // addFilas Gen_CAMPOS_GRID(1)
            addFilas: function() {
            var filas = [];
 filas.push({ NOMBRE: "REFERENCIA", VALOR1: "", VALOR2: "", CDATA: "0" }); 
 filas.push({ NOMBRE: "NOMBREOBJETO", VALOR1: "", VALOR2: "", CDATA: "0" }); 
        ...
        //LLAMA AL COMPONENTE EDIT PARA  AGREGAR UN REGISTRO NUEVO
         // RegistroNuevo Gen_CAMPOS_GRID(3)
            RegistroNuevo() {
                    ...
                    this.$store.commit("mutsetDatosDefault", defaultdatos);
                    //LLAVES
  this.dinamicKey.push({ nombre: "REFERENCIA", valor: "0" }); 
   //llama método editar registro
            eventBus.$emit("cargaRegistro", this.dinamicKey);
