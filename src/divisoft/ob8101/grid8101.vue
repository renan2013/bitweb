...
<template>
  <div>
   ...
   <table class="table">   
     ...
     <filtrosDivisoft v-show="activaFiltro" :header="locallastheader"  
     :isSidebarActive="activaFiltro" :filas="locallaststrFilas" 
     :filtros="filtros"  :ObjectID=8101   
...
export default {
    ...
    beforeMount() {
        ...
        this.columnDefs = datos8101.columnDefs;
        var defaultdatos = datos8101.datosDefault;
    ...
    methods: {
        ...
        onDoubleClicked(event) {
            ...
            this.dinamicKey = [];
 this.dinamicKey.push({ nombre: "ID_CATEGORIA", valor: event.node.data.ID_CATEGORIA });
      ...
      // addFilas Gen_CAMPOS_GRID(1)
            addFilas: function() {
            var filas = [];
 filas.push({ NOMBRE: "ID_CATEGORIA", VALOR1: "", VALOR2: "", CDATA: "0" }); 
        ...
        //LLAMA AL COMPONENTE EDIT PARA  AGREGAR UN REGISTRO NUEVO
         // RegistroNuevo Gen_CAMPOS_GRID(3)
            RegistroNuevo() {
                    ...
                    this.$store.commit("mutsetDatosDefault", defaultdatos);
                    //LLAVES
  this.dinamicKey.push({ nombre: "ID_CATEGORIA", valor: "0" }); 
   //llama método editar registro
            eventBus.$emit("cargaRegistro", this.dinamicKey);
