...
<template>
  ...
  <vx-card :title="getAction" code-toggler>
    ...
    <VuePerfectScrollbar class="scroll-area--data-list-add-new pt-4 pb-6"> 
      ...
      <p>Id Usuario</p>
        <vs-input size="large" v-validate="'required'"    name="ID_USUARIO" v-model="ID_USUARIO" class="mt-5 w-full" />
         
        <span class="text-danger">{{errors.first("step-1.ID_USUARIO")}}</span>
      </div><div class="vx-col w-1/2">
      <p>Id Rol</p>
        <vs-input size="large" v-validate="'required'"    name="ID_ROL" v-model="ID_ROL" class="mt-5 w-full" />
         
        <span class="text-danger">{{errors.first("step-1.ID_ROL")}}</span>
      </div> 
...
const dict = datos8106.mensajesValidacion;
...
export default {
  ...
  beforeMount() {
    ...
    this.defaultdatos = datos8106.datosDefault;
   ...
   data() {
    return {
      ...
      // CMS CMS CMS CMS          DatOS cms Para manejo de EXPEDIENTE DIGITAL  MULTIMEDIA Debe venir en una funcion de Datos con la consulta
        // CMSDATA8106: datos8106.CMSDATA8106,  PROPSDataCMS: divilib.DataCMS,  tituloAvatar: "Para Cambiar Avatar",  verDocumento1: false,
  ...
  watch: { 
   ...
   lastBdmsgcode:function(){
    ...
    "rowsForm": function() {
      if (this.rowsForm[0].ID_USUARIO > "0")
 ...
 validateStep3() {
   ...
   /////////// GUARDA DATOS GUARDA DATOS  GUARDA DATOS  GUARDA DATOS   ////////////      
    ...
    // CARGA DE una variable Bd a DOM
     cargaRegistroActual: function(datosRow) { 
          this.ID_USUARIO = datosRow.ID_USUARIO;
          this.ID_ROL = datosRow.ID_ROL;
   ...
   //cargaRegistroActual funcion completa   
    ...
    buscarRegistro: function(dinamicKey) {
      ...
      var origenUrl = "O";
       // agrego las filas
         var filas = datos8106.getMisFilas();
