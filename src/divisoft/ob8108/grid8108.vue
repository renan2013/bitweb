...
<template>
  <div>
   ...
   <table class="table">   
     ...
     <filtrosDivisoft v-show="activaFiltro" :header="locallastheader"  :isSidebarActive="activaFiltro" :filas="locallaststrFilas" :filtros="filtros"  :ObjectID=8108   @closeSidebar="activaFiltro = false" 
...
import * as datos8108 from "./js/divi8108.js";
...
export default {
    ...
    beforeMount() {
        ...
        this.columnDefs = datos8108.columnDefs;
        var defaultdatos = datos8108.datosDefault;
    ...
    methods: {
        ...
        onDoubleClicked(event) {
            ...
            this.dinamicKey = [];
 this.dinamicKey.push({ nombre: "IDADM_TAGS", valor: event.node.data.IDADM_TAGS });
      ...
      // addFilas Gen_CAMPOS_GRID(1)
            addFilas: function() {
            var filas = [];
 filas.push({ NOMBRE: "IDADM_TAGS", VALOR1: "", VALOR2: "", CDATA: "0" }); 
 filas.push({ NOMBRE: "NOMBRE", VALOR1: "", VALOR2: "", CDATA: "0" }); 
        ...
        //LLAMA AL COMPONENTE EDIT PARA  AGREGAR UN REGISTRO NUEVO
         // RegistroNuevo Gen_CAMPOS_GRID(3)
            RegistroNuevo() {
                    ...
                    this.$store.commit("mutsetDatosDefault", defaultdatos);
                    //LLAVES
  this.dinamicKey.push({ nombre: "IDADM_TAGS", valor: "0" }); 
   //llama método editar registro
            eventBus.$emit("cargaRegistro", this.dinamicKey);
